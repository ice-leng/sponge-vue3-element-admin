...
type MenuLogic interface {
	...
	Routes(ctx context.Context, roleIds common.LocalIntArray) ([]model.MenuItem, error)
	Options(ctx context.Context, request *types.OptionMenusRequest) ([]common.Options, error)
}
...
func (l menuLogic) Routes(ctx context.Context, roleIds common.LocalIntArray) ([]model.MenuItem, error) {
...
func (l menuLogic) Options(ctx context.Context, request *types.OptionMenusRequest) ([]common.Options, error) {
	tops, err := l.iDao.GetListByPid(ctx, 0, nil, request.OnlyParent)
	if err != nil {
		return make([]common.Options, 0), err
	}

	var items []common.Options
	for _, top := range tops {
...
func (l menuLogic) childrenOption(ctx context.Context, pid uint64, request *types.OptionMenusRequest) ([]common.Options, error) {
	items := make([]common.Options, 0)
